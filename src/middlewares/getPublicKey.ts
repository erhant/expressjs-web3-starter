 ...
 */
export async function getPublicKey(request: Request, response: Response, next: NextFunction) {
  const signature = request.headers[constants.Signatures.HeaderKey];

  if (signature === undefined || typeof signature !== 'string') {
    return respond.failure(response, 'No signature found.', StatusCodes.BAD_REQUEST);
  }
